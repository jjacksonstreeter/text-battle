...
# Provide a download button if CSV exists
try:
    if SCOREBOARD_PATH_CSV.exists():
        with open(SCOREBOARD_PATH_CSV, 'rb') as f:
            st.download_button("⬇️ Download scoreboard.csv", f, file_name="scoreboard.csv", mime="text/csv")
except Exception:
    pass

# Optional: allow clearing scoreboard locally
